...
async function adminCheckin(req, env, ch, ipHash) {
  ...
  if (code.includes('|')) {
    ...
    /* Manual entry always sends {regnum, event_code} with no `code` field.
       A scanned code with no "|" is a legacy plain-regnum badge (printed,
       or generated by a browser tab, before the event was encoded into the
       QR) -- fall back to the raw scanned text as the regnum instead of
       failing outright, and resolve the event below once the registration
       is known. */
